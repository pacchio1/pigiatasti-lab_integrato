...
CREATE DEFINER=`root`@`localhost` PROCEDURE `salva_evento`(IN `utente_id` INT, IN `data_inizio` DATETIME, IN `data_fine` DATETIME, IN `titolo` VARCHAR(255), IN `descrizione` VARCHAR(255), IN `max_partecipanti` INT, IN `posizione` VARCHAR(255), IN `categoria_id` INT)
BEGIN
    -- Set the delimiter to ;
    DECLARE CONTINUE HANDLER FOR SQLSTATE '45000' BEGIN END;

    -- Inserisci i valori nella tabella Evento
    SET @query = CONCAT('INSERT INTO Evento (id_utente, data_inizio, data_fine, titolo, descrizione, n_max_partecipanti, posizione, id_categoria)
    VALUES (', utente_id, ', ''', data_inizio, ''', ''', data_fine, ''', ''', titolo, ''', ''', descrizione, ''', ', max_partecipanti, ', ''', posizione, ''', ', categoria_id, ')');
